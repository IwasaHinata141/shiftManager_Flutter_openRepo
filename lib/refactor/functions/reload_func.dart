...
Future<List<dynamic>> calculateSalaryReload(
    focusedDay, hourlyWage, groupName, groupId, rowShift) async {
  // データ型変換のフォーマット指定
  ...
  for (int i = 0; i < groupId.length; i++) {
    ...
    // グループごとのシフトデータの取り出し
    Map<String, dynamic> shiftData = rowShift[groupId[i]];
    // 当月の日数分の繰り返し処理
    for (DateTime date = firstDay;
      ...
      if (dataField != null) {
        ...
        // 労働時間と時給から給与を計算
        final salary = (diffhour * double.parse(hourlyWage["${groupId[i]}"])).toInt();
        // 一日の給与を当月の給与に合計する
